...
function Activity2() {
  ...
  const [activityStartDate, setActivityStartDate] = useState(
    sessionStorage.getItem("活動日期") || ""
  );
  const [activityEndDate, setactivityEndDate] = useState(
    sessionStorage.getItem("活動結束日期") || ""
  );
  const [activityDeadLine, setActivityDeadLine] = useState(
    sessionStorage.getItem("報名截止日期") || ""
  );
  ...
  //付款方式
  const [activityPayment, setActivityPayment] = useState(
    sessionStorage.getItem("付款方式")
  );
  ...
  return user ? (
    <div className="activity_container">
      <div className="progressBar">
        <div className="progress12">1. 活動畫面</div>
        <div className="progress22">2. 細項資料</div>
        <div className="progress32">3. 預覽</div>
      </div>
